...
int preventing_pipe_at_end(char* pol){
    ...
    if(to_find != 0){
        syslog(LOG_ERR,"In function preventing_pipe_at_end():%s",strerror(errno));
        return -1;
    }
    int found = regexec(&regex2, pol, 0, NULL, 0);
        if(found == 0){
            syslog(LOG_ERR, "In function preventing_pipe_at_end(): character of pipe at the end of line");
            regfree(&regex2);
            return -1;
        }
        else if( found == REG_NOMATCH){
